...
export default async function PublicResumePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const ip =
    (await headers().catch(() => undefined))?.get("x-forwarded-for")?.split(",")[0]?.trim() ??
    "local";
      ...
      <main className="grid min-h-dvh place-items-center p-6 text-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            This page is being viewed very quickly. Try again in a minute.
          </p>
        </div>
      </main>
    );
  ...
  void db.usageEvent
    .create({ data: { event: "public_resume_view", props: { resumeId: resume.id } } })
    .catch(() => undefined);
  void db.resume
    .update({ where: { id: resume.id }, data: { viewCount: { increment: 1 } } })
    .catch(() => undefined);

  const owner = await db.careerProfile.findUnique({
    where: { id: resume.careerProfileId },
    select: { userId: true },
  });
  if (!owner) notFound();
  const { doc } = await loadResumeDocument(owner.userId, resume.id).catch(() => ({
    doc: null as never,
  }));
  if (!doc) notFound();
  ...
  return (
    <main className="min-h-dvh bg-neutral-200/70 py-10 dark:bg-neutral-950">
      <div className="mx-auto w-fit max-w-full">
        <div
          style={{ boxShadow: "0 10px 40px -12px rgb(0 0 0 / 0.35)" }}
