...
import { ImportResumeButton } from "@/features/import/components/import-button";
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 lg:px-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            {wantArchived ? "Archived resumes" : "Your resumes"}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {wantArchived
              ? "Restore any time — nothing is deleted while it's here."
              : "Each resume is a live view of your career profile."}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={wantArchived ? "/resumes" : "/resumes?archived=1"}
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          description={
            wantArchived
              ? "Archived resumes rest here until you restore them."
              : "Create one — your whole career profile comes along, pre-attached."
          }
