...
export default async function CoachPage() {
  ...
  const [profile, imports, matches, staleApps, pendingRecos, letters, applications, resumes] =
    await Promise.all([
      ...
      db.importJob.findMany({
        where: { userId: ctx.userId, status: "NEEDS_REVIEW" },
  ...
  if (!profile?.summary) {
    items.push({
      level: "act",
      title: "Your profile has no summary yet",
      detail: "One summary is shared by every resume — write it where it can be reused.",
      href: "/profile",
      cta: "Write summary",
    });
  }
  if ((profile?.experiences.length ?? 0) === 0) {
    items.push({
      level: "act",
      title: "No experience entries in your library",
      detail:
        "Resumes pull from your career library; capture your roles there once, reuse everywhere.",
      href: "/profile",
      cta: "Add experience",
    });
  }
  ...
  for (const m of matches) {
    ...
    if (gaps.length) {
      items.push({
        ...
        detail:
          "Only add what is genuinely true. If you have the experience elsewhere, put it in your library first.",
  ...
  for (const a of staleApps) {
    ...
    items.push({
      level: due ? "act" : "note",
      title: due
        ? `Follow-up with ${a.company} is ${Math.floor((now - new Date(a.followUpAt!).getTime()) / 86_400_000)}d overdue`
        : `Follow-up with ${a.company} on ${a.followUpAt!.toLocaleDateString()}`,
      detail: a.role,
  ...
  if (pendingRecos > 0)
    items.push({
      level: "note",
      title: `${pendingRecos} suggestion${pendingRecos > 1 ? "s" : ""} waiting on your decision`,
      detail:
        "Suggestions only surface facts already in your profile or rewrite your own wording — nothing applies itself.",
      href: "/jobs",
      cta: "Review",
    });
  if (applications > 0 && letters === 0)
    items.push({
      level: "note",
      title: "Applications tracked, but no draft letters",
      detail: "Letter scaffolds reuse only what your resume can back up.",
      href: "/cover-letters",
      cta: "Start one",
    });
  if (resumes > 0 && items.length === 0)
    items.push({
      level: "good",
      title: "Everything checks out",
      detail: "Profile populated, no imports awaiting review, no overdue follow-ups.",
    });
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 lg:px-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {acts > 0
            ? `${acts} thing${acts > 1 ? "s" : ""} worth doing now`
            : "A checklist from your own data — nothing invented, nothing naggy"}
        </p>
      </header>
      ...
      <p className="mt-6 text-xs text-muted-foreground">
        The coach never writes your resume for you. For AI rewrites of specific lines, use the AI
        button in the editor — always a review step first.
      </p>
    </div>
  );
}
