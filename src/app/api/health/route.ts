...
export async function GET(req: Request) {
  ...
  const privileged = session?.user?.id || isDevelopment() || ip === "localhost" || ip === "127.0.0.1";
  ...
  try {
    ...
    out.database = { ok: true, detail: env.NODE_ENV === "production" ? "connected" : `connected (${env.DATABASE_URL.replace(/:[^:@/]*@/, ":***@")})` };
  } catch (e) {
  ...
  const allOk = Object.values(out).every((v) => v.ok || v === out.ai || v === out.redis || v === out.mail);
