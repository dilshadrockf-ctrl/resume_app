  ...
  return (
    <Card className="overflow-hidden">
      <div className="relative h-[340px] overflow-hidden bg-muted/40">
        {html ? (
          <div className="pointer-events-none absolute left-1/2 top-3 origin-top -translate-x-1/2" style={{ pointerEvents: "none" }}>
            <div dangerouslySetInnerHTML={{ __html: html }} />
          </div>
        ) : (
          <div className="grid h-full place-items-center text-xs text-muted-foreground">preview when you add content</div>
        )}
        {current ? (
      ...
      <div className="p-4">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={template.ats === "excellent" ? "success" : template.ats === "good" ? "secondary" : "warning"}>ATS {template.ats}</Badge>
          {pages > 1 ? <Badge variant="warning" className="ml-auto">{pages} pages</Badge> : null}
        </div>
        ...
        {!current ? (
          ...
          <Button size="sm" variant="outline" className="mt-3 w-full" onClick={() => router.push(`/resumes/${resumeId}`)}>
            Open editor
          </Button>
        )}
      </div>
