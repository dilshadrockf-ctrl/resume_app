...
export function ProfileContactCard({ initial, summary }: { initial: Values; summary: string }) {
  ...
  const fields: Array<[keyof Values, string, string]> = [
    ...
    ["headline", "Headline", "Senior Platform Engineer"],
    ["email", "Email", ""],
    ["phone", "Phone", ""],
    ["location", "Location", "City, State"],
    ["website", "Website", "yoursite.dev"],
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Printed on every resume. Only fields you fill are shown on documents.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3">
        ...
        <Field
          ...
          hint="2–4 sentences. Facts only — the AI assistant can polish wording, never invent history."
        >
          <Textarea rows={4} value={s} onChange={(e) => setS(e.target.value)} />
        </Field>
        <div className="flex justify-end">
          <Button onClick={() => void save()} disabled={busy}>
            {busy ? <Spinner className="size-4 animate-spin" /> : <Save className="size-4" />} Save
            profile
          </Button>
        </div>
      </CardContent>
