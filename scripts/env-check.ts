...
const fmt = (label: string, ok: boolean, detail = "") =>
  console.log(
    `${ok ? "\x1b[32m✔\x1b[0m" : "\x1b[33m•\x1b[0m"} ${label}${detail ? ` — ${detail}` : ""}`,
  );
...
fmt(
  ...
  env.AUTH_SECRET ? `${env.AUTH_SECRET.length} chars` : "missing (dev-only tolerance)",
);
fmt(
  "redis queue",
  Boolean(env.REDIS_URL),
  env.REDIS_URL ? "dedicated worker available" : "in-process queue (single node)",
);
const st = storageStatus();
fmt("file storage", st.ok, `${st.driver}: ${st.detail.slice(0, 80)}`);
fmt(
  "mail",
  Boolean(env.MAIL_HOST),
  env.MAIL_HOST ?? "not set — signup emails disabled, app still works",
);
const ai = appConfig.aiConfigured ? true : false;
fmt(
  "AI provider",
  ai,
  ai ? `${env.AI_PROVIDER}/${env.AI_DEFAULT_MODEL ?? "default"}` : "none — optional by design",
);
fmt(
  "billing",
  env.BILLING_ENABLED,
  env.BILLING_ENABLED ? "ENABLED" : "dormant (architecture ready, no payment flows)",
);
